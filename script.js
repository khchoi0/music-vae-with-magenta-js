...
function playNote(midiNote, numNoteHolds) {
	...
	player.buffer = pianoSamples.get(midiNote);
	player.fadeOut = 0.05;
	player.fadeIn = 0.01;
...
function draw() {
	...
	drawPlayhead(
		new p5.Vector(tileLeft(0) + percent * calcCurrentSequenceWidth(), SCRUB_BAR_HEIGHT),
		SCRUB_BAR_GAP
	);
}
...
function togglePlayback() {
	if (!interpolatedNoteSequences || !pianoSamples) {
		return;
	}
	if (Tone.Transport.state === 'started') {
